...
def gumbel_distribution(input_shape: Tuple[int, ...]) -> tf.Tensor:
    ...
    gumbel_dist = -1 * tf.math.log(
        -1 * tf.math.log(uniform_dist + TOL) + TOL
    )
    return gumbel_dist
...
class GumbelSoftmax(Layer):
    ...
    def call(self, inputs: tf.Tensor) -> Tuple[tf.Tensor]:
        x = (inputs + gumbel_distribution(tf.shape(inputs)))/temperature
        return x
...
def load_dataset() :
    ...
    print ('Train Validation : X=%s, Y=%s'%(trainValX.shape,trainValY.shape))    
...
def cnn_gumbelsoftmax_model():
    basemodelgumbel=GumbelSoftmax(input_shape=(32,32,3))
    model = Sequential()
...
def run_test_harness_gumbelsoftmax_cnn() :
...
run_test_harness_gumbelsoftmax_cnn()   
